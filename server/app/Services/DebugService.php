...
class DebugService extends ExceptionHandler
{
    use RestApi;
    const ERRTYPE = "Server Side Error";
    ...
    public function sendPhpResponse($exception)
    {
        ...
        $this->finishSendingRestfulException($exception);

    }

    public function finishSendingRestfulException(FlattenException $exception)
    {
        ...
        $exceptionArr = $exception->toArray();

        $this->sendJSON([
            "createdAt" => $datetime,
            "type" => self::ERRTYPE,
            "__exception" => $exception->toArray()
        ]);

        $exception = "\n" . $datetime." - ".self::ERRTYPE . "\n";

        try {

        //error_log("Error", 3, "./store/.log");
        foreach ($exceptionArr as $position => $e) {

            $this->total = $count + 1;
            $this->lass = $e['class'];
            $this->message = $e['message'];
            //ind
            $this->current = $count - $position + 1;


            //$message = ltrim($e['message']);
            //nl2br(

            $exception .= sprintf("Text muted: (%d/%d)\n
            Exception title: %s\n
            Trace message: %s\n",
            $this->current, $this->total, $this->class, $this->message);
            ...
            foreach ($e['trace'] as $trace) {
                ...
                if ($trace['function']) {
                    ...
                    foreach ($trace['args'] as $key => $arg){
                        ...
                        DB::table("arguments")->create([
                            "type" => $key,
                            "class" => $arg,
                        ])->createWithByOppositeForeignKey("stack_trace_id")->finish();
                    }
                ...
                DB::table("stack_traces")->create([
                    "namespace" => $trace['namespace'],
                    "short_class" => $trace['short_class'],
                    "class" => $trace['class'],
                    "type" => $trace['type'],
                    "function" => $trace['function'],
                    "file" =>  isset($trace['file'])? $trace['file'] : null,
                    "line" => isset($trace['line']) ? $trace['line'] : null,
                ])->createWithByOppositeForeignKey("server_exception_id")->finish();
            ...
            $exception .= "\n";
                ...
                if ($this->debug) {
                    ...
                    $exception .= sprintf("Exception thrown when handling an exception (%s: %s)", $e->getClass(), $this->escapeHtml($e->getMessage()));

                    DB::table("unsafe_exceptions")->create([
                        "class" => $e->getClass(),
        ...
        error_log($exception, 3, __DIR__  . "/../../store/.log");
        error_log($exception, 3, __DIR__  . "/../../store/server.log");
        ...
        $this->sendJSON(["errorMessage" => dd( DB::schema()->getErrorMessage())]) ;
        ...
        $stackTrace= [[
            "namespace" => "",
                    ...
                    "type" => "",
                    "function" =>"",
                    "file" => "E:\\soft\\eshop-sweetheart\\server\\app\\Controllers\\WelcomeController.php",
                    "line" => 19,
                    "args" => [],
        ]];
